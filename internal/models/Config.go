...
import (
	...
	"github.com/ltkh/montools/internal/models/units"
...
func InitConfig(file, defaultPath string) *Config {
	...
	if created {
		log.Printf("[error] config %s created successfully", file)
		os.Exit(0)
		return nil
	}
...
func (config *Config) LoadRoutes() ([]Route, error) {
	...
	for _, sRoute := range config.RouteFiles {
		...
		if !route.LoadAddress(config) {
			log.Printf("[warn] at least one address used in '%s' was not found. This Route/Address might be unavailable", sRoute)
			continue
		}
