...
func (route Route) Check(config *Config) bool {
	...
	if route.NeedSSL() {
		if len(route.SSL.Cert) == 0 || len(route.SSL.Key) == 0 {
			log.Print("[error] missing SSL config: SSL cert or key")
			return false
		}

		// Check cert exists
		if !gaw.FileExists(route.SSL.Cert) {
			log.Printf("[error] SSL cert file '%s' not found", route.SSL.Cert)
			return false
		}

		// Check key exists
		if !gaw.FileExists(route.SSL.Key) {
			log.Printf("[error] SSL key file '%s' not found", route.SSL.Key)
			return false
		}
	}
	...
	for _, location := range route.Locations {
		if !isURLValid(location.Destination) {
			log.Printf("[error] Location in %s is malformed!\n", route.FileName)
			return false
		}
		...
		if isHostsAddress(location.DestinationURL.Hostname()) && gaw.IsInStringArray(port, location.Ports()) {
			log.Print("[error] request loop detected")
			return false
		}
	}
...
func FindMatchingLocation(routes []*Route, r *http.Request) *RouteLocation {
	...
	for _, route := range routes {
		...
		if found != nil {
			log.Print(r.URL, " -> ", found.DestinationURL)
			found.Route = route
			return found
		}
		...
		if route.DefaultLocation != nil {
			log.Print(r.URL, " -> ", route.DefaultLocation.DestinationURL)
