...
import (
	...
	"github.com/ltkh/montools/internal/models"
)
	...
	for i, listenAddress := range server.Config.ListenAddresses {
		...
		if listenAddress.SSL {
			...
			if len(certKeyPairs) == 0 {
				log.Printf("[warn] couldn't find any certificate pairs for Address '%s'. This Route/Server might be unavailable", listenAddress.Address)
				continue
			}

			tlsConfig := &tls.Config{}
			for _, pair := range certKeyPairs {
				log.Print("[debug] found cert: ", pair.Cert, " Key: ", pair.Key)
...
func (server *ReverseProxyServer) WaitForShutdown() {
	...
	log.Print("[info] shutting down server")

	for i := range server.Server {
		server.Server[i].Server.Shutdown(ctx)
	}

	log.Print("[info] shutting down complete")
	os.Exit(0)
}
